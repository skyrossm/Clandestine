...
public class ArcaneChunkGenerator implements IChunkGenerator {


	//Block that replaces End Stone
	protected static final IBlockState MAIN_BLOCK = ModBlocks.ERTHA_BLOCK.getDefaultState();
    ...
    public ArcaneChunkGenerator(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn)
    {
        this.world = world;
        ...
        ArcaneChunkGenerator.islandNoise = new NoiseGeneratorSimplex(this.rand);
        ...
        ArcaneChunkGenerator.islandNoise = ctx.getIsland();
    ...
    public void setBlocksInChunk(int x, int z, ChunkPrimer primer)
    {
        ...
        this.buffer = this.getHeights(this.buffer, x * 2, 0, z * 2, 20, 50, 20);
        ...
        for (int i1 = 0; i1 < 2; ++i1)
        {
            for (int j1 = 0; j1 < 2; ++j1)
            {
                ...
                {
                    double d1 = this.buffer[((i1 + 0) * 20 + j1 + 0) * 50 + k1 + 0];
                    double d2 = this.buffer[((i1 + 0) * 20 + j1 + 1) * 50 + k1 + 0];
                    double d3 = this.buffer[((i1 + 1) * 20 + j1 + 0) * 50 + k1 + 0];
                    double d4 = this.buffer[((i1 + 1) * 20 + j1 + 1) * 50 + k1 + 0];
                    double d5 = (this.buffer[((i1 + 0) * 20 + j1 + 0) * 50 + k1 + 1] - d1) * 0.25D;
                    double d6 = (this.buffer[((i1 + 0) * 20 + j1 + 1) * 50 + k1 + 1] - d2) * 0.25D;
                    double d7 = (this.buffer[((i1 + 1) * 20 + j1 + 0) * 50 + k1 + 1] - d3) * 0.25D;
                    double d8 = (this.buffer[((i1 + 1) * 20 + j1 + 1) * 50 + k1 + 1] - d4) * 0.25D;
    ...
    public Chunk generateChunk(int x, int z)
    {
        ...
        {
            abyte[i] = (byte)Biome.getIdForBiome(ModBiomes.ERTHA);
        }
    ...
    public static float getIslandHeightValue(int x, int z, int p_185960_3_, int p_185960_4_)
    {
        ...
        for (int i = -12; i <= 12; ++i)
        {
            for (int j = -12; j <= 12; ++j)
            {
                ...
                //If distance from center is far enough
                if (k * k + l * l > 4096L && ArcaneChunkGenerator.islandNoise.getValue((double)k, (double)l) < -0.8999999761581421D)
                {
    ...
    {
        return (long)x * (long)x + (long)z * (long)z > 4096L && ArcaneChunkGenerator.getIslandHeightValue(x, z, 1, 1) >= 0.0F;
    }
    ...
    private double[] getHeights(double[] noiseField, int x, int y, int z, int xSize, int ySize, int zSize)
    {
        ...
        for (int l = 0; l < xSize; ++l)
        {
            ...
            {
                float f = ArcaneChunkGenerator.getIslandHeightValue(i, j, l, i1);
    ...
    public void populate(int x, int z)
    {
        ...
        long distanceFromCenter = (long)x * (long)x + (long)z * (long)z;

        
        if (distanceFromCenter > 4096L)
        {

            if (ArcaneChunkGenerator.getIslandHeightValue(x, z, 1, 1) > 40.0F)
            {
